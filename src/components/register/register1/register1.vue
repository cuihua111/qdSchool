  ...
  <div class="register1">
   <el-form
        ...
        <el-form-item
            label="手机号码"
            prop="mobile">
        <el-input type="mobile" v-model.text="numberValidateForm.mobile" autocomplete="off" placeholder="请输入手机号码"></el-input>
        </el-form-item>
          <el-form-item
            label="短信验证码"
            prop="code">
          <el-input type="code" v-model.text="numberValidateForm.code" autocomplete="off" placeholder="请输入验证码"></el-input>
      ...
      return {
        ...
        rules:{
            mobile:[
              ...
              { type: 'string', message: '手机号码必须为数字'}
            ],
            code:[
              { required: true, message: '验证码不能为空'},
              { type: 'string', message: '验证码必须为数字'}
            ]
        }
      }
    },
    ...
    methods: {
        ...
        submitForm(formName) {
            ...
            this.$refs[formName].validate((valid) => {
              if (valid) {
                postAxios({url:'/singcampus/checkPhonenumOnRegisterSchool',data:{phonenum:this.numberValidateForm.mobile,checkcode:this.numberValidateForm.code}})
                .then((res) => {
